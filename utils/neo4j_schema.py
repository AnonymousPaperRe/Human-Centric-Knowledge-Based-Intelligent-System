...
#### Queries ####
"""
label: Label of the node (if applicable)
other: Related nodes for relationships (not used in this case)
elementType: Type of the element ("node" or "relationship")
type: Type of the property ("INTEGER","FLOAT","LIST","STRING","BOOLEAN or "relationship")
...
class Neo4jSchema(Neo4jGraph):
    ...
    def extract_all_node_instances(self, 
                            selected_labels: List[str] 
                            ) -> List[Any]:
        """
        Function to extract all node instances: attributes & values."""
        extracted = []
        for label in selected_labels:
            query_nodes = f"""MATCH (p:{label}) 
                            RETURN {{Label: '{label}', properties: properties(p)}} AS Instance
    ...
    def extract_all_relationship_instances(self,
                                       rel: Dict) -> List[Any]:
        """
        Function to extract instances for a given relationship, written as a triple.
        The data includes properties for both nodes and relationship (if any).
    ...
    def extract_all_multiple_relationships_instances( self,
                            rtriples: List[Any]) -> List[Any]:
        """Extracts all instances of each from a relationships list."""
        extracted = []
        for rtriple in rtriples:
            temp_list = self.extract_all_relationship_instances(rtriple)
            extracted.append(temp_list)
        return extracted
