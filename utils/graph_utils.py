...
#### RELATIONSHIPS ####
def get_relationship_properties(jschema: Dict,
                        label: str,
                        datatypes: bool=False,
                        datatype: str=""
                        ) -> Any:
    """Function to extract a list of properties for a given relationship.
    Options to return the datatypes or a properties of specific datatype only."""
   
    node_info = jschema['rel_props'][label]
    if datatypes:
        if len(datatype) > 1:
            props = [el['property'] for el in node_info if el['datatype'] == datatype]
        else:
...
def filter_relationships_instances_with_rela_properties(jschema: Dict,
                                   INSTANCES_RELS: List[Dict],
                                   datatype_start: str,
                                   datatype_end: str
                                   )-> List[Dict]:
    ...
    for coll in INSTANCES_RELS:
        for instance in coll:
            ...
            rel = triple[1]
            selected_props_rel = get_relationship_properties(jschema, rel, True, datatype_start)  # Assuming relationship props are strings
            rel_props = extract_subdict(instance[triple[1]], selected_props_rel) 

            if selected_start and selected_end:
                result.append([label_start, selected_start, rel, rel_props, label_end, selected_end])
    return result
